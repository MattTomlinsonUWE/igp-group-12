...
st.set_page_config(
    page_title="Stock Seasonality Application - Christmas Holiday Strategies",
...
st.write("## 'Christmas Holiday' Strategies:")

st.subheader("Strategy overview:")
st.write("Buy the stock before Christmas and sell after Christmas holiday.")
st.write("Hence in this analysis, pattern of returns has been observed between 20th Decemer & 2nd January between 2004 to 2025.")
...
def fetch_stock_data(ticker):
    ...
    filtered_data = pd.concat([
        data[(data['Date'] >= pd.Timestamp(f'{year}-12-20')) & (data['Date'] <= pd.Timestamp(f'{year+1}-01-02'))]
...
def calculate_return_stats(df, annualized_return, cumulative_return):
    ...
    st.write("By analyzing historical returns, one can gain a better understanding of seasonality effects during Christmas holiday.")
...
st.subheader("Stock ticker details:")
